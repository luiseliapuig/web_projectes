...
// Tipos técnicos ordenados dentro de cada categoría; la familia se obtiene
// mediante JOIN a través de la categoría, nunca se guarda en el tipo.
$stmt = $pdo->query("
    SELECT tp.id_tipo_proyecto, tp.nombre, tp.activo, tp.orden,
           cp.nombre AS categoria, f.nombre AS familia
    FROM app.proyecto_tipos tp
    INNER JOIN app.proyecto_categorias cp ON cp.id_categoria_proyecto = tp.categoria_proyecto_id
    INNER JOIN app.familias_ciclos f ON f.id_familia_ciclo = cp.familia_ciclo_id
    ORDER BY f.orden, f.nombre, cp.orden, cp.nombre, tp.activo DESC, tp.orden, tp.nombre
");
...
<div class="container-fluid py-4">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 mb-3"><div><h1 class="h3 mb-1">Tipus de projectes</h1><p class="text-muted mb-0">Naturalesa tècnica del projecte per categoria.</p></div><a href="/index.php?main=tipus-projectes_form&amp;modo=new" class="btn btn-puig-solid rounded-pill px-4">Nou tipus</a></div>
    ...
    <div class="card shadow-sm border-0 rounded-4 overflow-hidden"><div class="table-responsive"><table class="table table-hover align-middle mb-0 tipus-projectes-table">
        <thead class="table-light"><tr><th class="ps-4">Tipus</th><th>Categoria</th><th>Família</th><th class="text-center">Ordre</th><th class="text-center">Actiu</th><th class="text-end pe-4">Accions</th></tr></thead>
        <tbody>
        <?php foreach ($tipos as $tipo): ?><tr>
            <td class="ps-4 fw-semibold"><?= htmlspecialchars((string) $tipo['nombre'], ENT_QUOTES, 'UTF-8') ?></td><td><?= htmlspecialchars((string) $tipo['categoria'], ENT_QUOTES, 'UTF-8') ?></td><td><?= htmlspecialchars((string) $tipo['familia'], ENT_QUOTES, 'UTF-8') ?></td><td class="text-center"><?= (int) $tipo['orden'] ?></td>
        ...
        <?php if ($tipos === []): ?><tr><td colspan="6" class="text-center text-muted py-5">No hi ha tipus creats.</td></tr><?php endif; ?>
        </tbody>
    </table></div></div>
</div>
