...
$curs = isset($_GET['curs']) && is_string($_GET['curs'])
    ? trim($_GET['curs'])
    : cursoAcademicoDefensas();
if (!in_array($curs, $cursos, true)) {
    $curs = cursoAcademicoDefensas();
}
if (!in_array($curs, $cursos, true)) {
    array_unshift($cursos, $curs);
}
...
$sql = "
    ...
    WHERE p.defensa_fecha IS NOT NULL
      AND p.curso_academico = :curs
    GROUP BY p.id_proyecto, p.nombre, c.abr, g.grupo,
             p.defensa_fecha, a.codigo, a.nombre
    ORDER BY p.defensa_fecha ASC
";
$stmt = $pdo->prepare($sql);
$stmt->bindValue(':curs', $curs);
$stmt->execute();
