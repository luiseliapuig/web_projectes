...
// El calendario se consulta por familia, manteniendo todas las aulas como
// posibles destinos para que el ajuste manual siga siendo completamente libre.
...
<section class="container-fluid py-4 px-4">
    ...
    <div class="d-flex flex-wrap align-items-center gap-3 mb-4">
        <div>
            ...
            <div class="small fw-semibold">Curs <?= htmlspecialchars($cursoAcademico, ENT_QUOTES, 'UTF-8') ?></div>
        </div>
        <div class="ms-auto d-flex gap-2 align-items-center flex-wrap">
            <select class="form-select form-select-sm" id="familia-drag" aria-label="Família professional">
...
// ── Estat ─────────────────────────────────────────────────────────
let _torn        = 'tarda';
let _familiaId   = document.getElementById('familia-drag').value;
let _dies        = [];
let _aules       = [];
let _hores       = [];
let _duracio_min = 45;
...
async function carregar() {
    document.getElementById('drag-wrap').innerHTML =
        '<div class="text-center text-muted py-5">Carregant...</div>';
    try {
        const r = await fetch(`/index.php?main=calendari_drag_dades&torn=${_torn}&curso=${encodeURIComponent(cursoAcademico)}&familia=${encodeURIComponent(_familiaId)}`);
...
function render() {
    ...
    // Solo se muestran franjas reales de la propuesta. Así el descanso no se
    // interpreta erróneamente como una defensa de duración extraordinaria.
    const horesRender = [..._hores].sort();
...
async function guardar(from, toDia, toHora, toAulaId) {
    ...
    try {
        ...
        fd.append('curso_academico', cursoAcademico);
        fd.append('csrf_token', csrfToken);

        const r = await fetch('/index.php?main=calendari_modificar', {
            method: 'POST',
...
document.getElementById('familia-drag').addEventListener('change', function () {
    _familiaId = this.value;
    carregar();
});
