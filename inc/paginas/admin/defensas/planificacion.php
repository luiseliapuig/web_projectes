...
// Familias activas disponibles para acotar la futura planificación.
$stmtFamilias = $pdo->query("
    SELECT id_familia_ciclo, nombre
    FROM app.familias_ciclos
    WHERE activo = true
    ORDER BY orden, nombre
");
$familiasCiclos = $stmtFamilias->fetchAll(PDO::FETCH_ASSOC);
// planificacio.php
        ...
        <div class="card-body p-4 p-lg-5">
            <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3">
                <div>
                    ...
                    <div class="mt-2 fw-semibold">Curs <?= htmlspecialchars($cursoAcademico, ENT_QUOTES, 'UTF-8') ?></div>
                </div>
                <div class="d-flex flex-wrap gap-2">
                    <span class="badge rounded-pill text-bg-light border px-3 py-2">5 dies</span>
        ...
        <div class="col-12 col-xl-8">
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                ...
                <div class="card-body px-4 pb-4 pt-3">
                    <div class="mb-4">
                        <label for="familia_ciclo_id" class="form-label fw-semibold">Família professional</label>
                        <select class="form-select rounded-3" id="familia_ciclo_id" name="familia_ciclo_id" required>
                            <option value="">Selecciona una família</option>
                            <?php foreach ($familiasCiclos as $familia): ?>
                    ...
                    <div class="row row-cols-1 row-cols-md-3 row-cols-xl-5 g-3">
                        ...
                        <div class="col">
                            <label for="dia4" class="form-label fw-semibold">Dia 4</label>
                            <input type="date" class="form-control rounded-3" id="dia4" name="dia4">
                        </div>
                        <div class="col">
                            <label for="dia5" class="form-label fw-semibold">Dia 5</label>
                            <input type="date" class="form-control rounded-3" id="dia5" name="dia5">
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="col-12 col-lg-6">
                            <div class="border rounded-4 p-4 h-100 bg-light-subtle">
                                ...
                                <div class="border-top mt-4 pt-3">
                                    <div class="fw-semibold mb-3">Descans</div>
                                    <div class="row g-3">
                                        <div class="col-12 col-md-6">
                                            <label for="descans_inici_mati" class="form-label">Hora inici</label>
                                            <input type="time" class="form-control rounded-3" id="descans_inici_mati" name="descans_inici_mati" value="11:10">
                                        </div>
                                        <div class="col-12 col-md-6">
                                            <label for="descans_fi_mati" class="form-label">Hora fi</label>
                                            <input type="time" class="form-control rounded-3" id="descans_fi_mati" name="descans_fi_mati" value="11:40">
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="col-12 col-lg-6">
                            <div class="border rounded-4 p-4 h-100 bg-light-subtle">
                                ...
                                <div class="border-top mt-4 pt-3">
                                    ...
                                    <div class="row g-3">
                                        <div class="col-12 col-md-6">
                                            <label for="descans_inici_tarda" class="form-label">Hora inici</label>
                                            <input type="time" class="form-control rounded-3" id="descans_inici_tarda" name="descans_inici_tarda" value="18:00">
                                        </div>
                                        <div class="col-12 col-md-6">
                                            <label for="descans_fi_tarda" class="form-label">Hora fi</label>
                                            <input type="time" class="form-control rounded-3" id="descans_fi_tarda" name="descans_fi_tarda" value="18:20">
                                        </div>
                                    </div>
                                </div>
...
(function () {
    ...
    const csrfToken = <?= json_encode(tokenCsrf()) ?>;
    const cursoAcademico = <?= json_encode($cursoAcademico) ?>;

    function recollirDades() {
        return {
            familia_ciclo_id: document.getElementById('familia_ciclo_id').value,
            ...
            descans_inici_mati: document.getElementById('descans_inici_mati').value,
            descans_fi_mati:    document.getElementById('descans_fi_mati').value,
            hora_inici_tarda: document.getElementById('hora_inici_tarda').value,
            hora_fi_tarda:    document.getElementById('hora_fi_tarda').value,
            descans_inici_tarda: document.getElementById('descans_inici_tarda').value,
            descans_fi_tarda:    document.getElementById('descans_fi_tarda').value,
    ...
    function validarDades(dades) {
        if (!dades.familia_ciclo_id) return 'Has de seleccionar una família professional.';
        const diesSeleccionats = [dades.dia1, dades.dia2, dades.dia3, dades.dia4, dades.dia5].filter(Boolean);
        if (diesSeleccionats.length < 1) return 'Has d\'indicar com a mínim un dia de defensa.';
        if (!dades.hora_inici_mati || !dades.hora_fi_mati) return 'Has d\'indicar l\'horari del torn de matí.';
        if (!dades.descans_inici_mati || !dades.descans_fi_mati) return 'Has d\'indicar el descans del torn de matí.';
        if (!dades.hora_inici_tarda || !dades.hora_fi_tarda) return 'Has d\'indicar l\'horari del torn de tarda.';
        if (!dades.descans_inici_tarda || !dades.descans_fi_tarda) return 'Has d\'indicar el descans del torn de tarda.';
        return null;
    }
    ...
    document.getElementById('btn-eliminar').addEventListener('click', async function () {
        if (!confirm(`Esborraràs totes les dates i aules de defensa del curs ${cursoAcademico}. Continuar?`)) return;

        setBotoCarregant(this, true);
        setLog('Eliminant dates...');

        try {
            const resp = await fetch('/index.php?main=planificacio_eliminar', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ curso_academico: cursoAcademico, csrf_token: csrfToken }),
            });
    ...
    document.getElementById('btn-simular').addEventListener('click', async function () {
        ...
        try {
            const resp = await fetch('/index.php?main=planificacio_simular', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    document.getElementById('btn-aplicar').addEventListener('click', async function () {
        ...
        try {
            const resp = await fetch('/index.php?main=planificacio_accion', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(dades),
            });
