...
// Les notes finals són comunes a tot el professorat quan la regla està activa.
// El superadministrador conserva l'accés per a supervisar la informació.
if (
    !esProfesor()
    || (!configuracion('mostrar_notes_finals') && !esSuperadmin())
) {
    http_response_code(403);
    die('Accés no permès');
}

// Les famílies es dedueixen dels grups assignats al professor en el curs vigent.
$cursoActual = cursoAcademicoActual();
$profesorId = (int) $_SESSION['professor_id'];
$stmtCiclos = $pdo->prepare("
    WITH familias_profesor AS (
        SELECT DISTINCT c.familia_ciclo_id
        ...
        WHERE rpg.profesor_id = :profesor_id
          AND rpg.curso_academico = :curso_actual
    )
    SELECT c.id_ciclo, c.abr, c.nombre, c.color
    FROM app.ciclos c
    INNER JOIN familias_profesor fp
        ON fp.familia_ciclo_id = c.familia_ciclo_id
    WHERE c.activo = true
    ORDER BY c.orden, c.abr
");
$stmtCiclos->execute([
    ':profesor_id' => $profesorId,
    ':curso_actual' => $cursoActual,
]);
$ciclesDisponibles = $stmtCiclos->fetchAll(PDO::FETCH_ASSOC);
$abreviaturasValidas = array_column($ciclesDisponibles, 'abr');

// El selector conserva los cursos históricos y muestra siempre el vigente.
$cursosDisponibles = $pdo->query("
    SELECT DISTINCT curso_academico
    FROM app.proyectos
    WHERE curso_academico ~ '^[0-9]{4}-[0-9]{2}$'
    ORDER BY curso_academico DESC
")->fetchAll(PDO::FETCH_COLUMN);
if (!in_array($cursoActual, $cursosDisponibles, true)) {
    array_unshift($cursosDisponibles, $cursoActual);
}

$cursoSeleccionado = isset($_GET['curso']) && is_string($_GET['curso'])
    ? trim($_GET['curso'])
    : $cursoActual;
if (!in_array($cursoSeleccionado, $cursosDisponibles, true)) {
    $cursoSeleccionado = $cursoActual;
}

$cicleActiu = isset($_GET['cicle']) && is_string($_GET['cicle'])
    ? strtoupper(trim($_GET['cicle']))
    : '';

// El filtre només accepta cicles de les famílies accessibles al professor.
if (!in_array($cicleActiu, $abreviaturasValidas, true)) {
    $cicleActiu = $abreviaturasValidas[0] ?? '';
}
$cicloIdActivo = 0;
foreach ($ciclesDisponibles as $cicloDisponible) {
    if ($cicloDisponible['abr'] === $cicleActiu) {
        $cicloIdActivo = (int) $cicloDisponible['id_ciclo'];
        break;
    }
}
...
INNER JOIN app.grupos                 gp  ON gp.id_grupo = p.grupo_id
INNER JOIN app.ciclos                 cp  ON cp.id_ciclo = gp.id_ciclo
...
GROUP BY
    ...
    cp.abr, gp.grupo, p.curso_academico, p.defensa_fecha, p.defensa_aula_id,
...
ORDER BY gp.grupo, p.nombre
";

$stmt = $pdo->prepare($sql);
$stmt->execute([
    ':ciclo_id' => $cicloIdActivo,
    ':curso_academico' => $cursoSeleccionado,
]);
$projectes = [];
...
<script>
window.PAGE_TITLE = 'Notes finals';
</script>

<div class="container-fluid">
<div class="panel-entregues professor-content-panel">

    <div class="projectes-header mb-4 mt-30">
        <h1 class="projectes-title mb-2">Notes finals</h1>
        <p class="projectes-subtitle mb-0">Revisió de l'estat dels projectes i de les qualificacions finals.</p>
    </div>
    ...
    <div class="projectes-filter mb-4">
        <div class="d-flex flex-wrap gap-2">
            <?php foreach ($ciclesDisponibles as $cicle): ?>
                <a
                    href="/notes-finals/<?= rawurlencode((string) $cicle['abr']) ?>?curso=<?= rawurlencode($cursoSeleccionado) ?>"
                    class="projectes-filter-pill <?= $cicle['abr'] === $cicleActiu ? 'active' : '' ?>"
                ><?= h((string) $cicle['abr']) ?></a>
            <?php endforeach ?>
        </div>
    </div>

    <!-- El curs vigent és el valor inicial, però es conserva l'accés històric. -->
    <form method="get" action="<?= $cicleActiu !== '' ? '/notes-finals/' . rawurlencode($cicleActiu) : '/notes-finals' ?>" class="row g-2 align-items-end mb-4">
        <div class="col-sm-6 col-md-4 col-lg-3">
            ...
            <select name="curso" id="curso" class="form-select" onchange="this.form.submit()">
                <?php foreach ($cursosDisponibles as $cursoDisponible): ?>
                    <option value="<?= h((string) $cursoDisponible) ?>" <?= $cursoDisponible === $cursoSeleccionado ? 'selected' : '' ?>>
                        <?= h((string) $cursoDisponible) ?>
                    </option>
                <?php endforeach; ?>
            </select>
        </div>
        <noscript>
            <div class="col-auto"><button type="submit" class="btn btn-puig">Aplicar</button></div>
        </noscript>
    </form>

    <?php if ($ciclesDisponibles === []): ?>
        <div class="alert alert-info">No tens cap família professional assignada en el curs actual.</div>
    <?php endif; ?>
