...
// L'acció repeteix l'autorització perquè no confia només en el router.
// El superadministrador conserva la gestió amb la selecció tancada.
if (
    !esProfesor()
...
// La càrrega JSON es valida abans de permetre qualsevol escriptura.
$input     = json_decode(file_get_contents('php://input'), true) ?? [];
$tokenCsrf = $input['csrf_token'] ?? null;

if (!validarTokenCsrf($tokenCsrf)) {
    http_response_code(403);
    echo json_encode(['ok' => false, 'missatge' => 'Token de seguretat no vàlid']);
    exit;
}
...
try {
    ...
    error_log('Error en modificar un tribunal: ' . $e->getMessage());
    http_response_code(500);
    echo json_encode(['ok' => false, 'missatge' => 'No s\'ha pogut modificar el tribunal']);
}
exit;
