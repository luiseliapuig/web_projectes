...
// El endpoint repeteix l'autorització perquè no confia només en el router.
// El superadministrador pot consultar-lo encara que la selecció estiga tancada.
if (
    !esProfesor()
...
try {
    ...
    $projectes = $pdo->query("
        SELECT
            p.id_proyecto, p.uuid, p.nombre, c.abr AS ciclo, g.grupo,
            p.defensa_aula_id,
    ...
    error_log('Error en consultar el calendari de tribunals: ' . $e->getMessage());
    http_response_code(500);
    echo json_encode(['ok' => false, 'missatge' => 'No s\'ha pogut carregar el calendari']);
}
